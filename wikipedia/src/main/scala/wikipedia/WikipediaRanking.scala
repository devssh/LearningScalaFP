...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)(
    (acc, wikiArticle) => if (wikiArticle.mentionsLanguage(lang)) acc + 1 else acc,
    _ + _
  )
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(wikiArticle => {
      langs.filter(lang => wikiArticle.mentionsLanguage(lang)).map((_, wikiArticle))
    }).groupByKey()
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(pair => (pair._1, pair._2.size)).collect().sortBy(_._2).toList.reverse
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(wikiArticle => {
      langs.filter(lang => wikiArticle.mentionsLanguage(lang)).map((_, 1))
    }).reduceByKey(_+_).collect().sortBy(_._2).toList.reverse
  }
