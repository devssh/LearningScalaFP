...
object TimeUsage {
  ...
  /** @return The schema of the DataFrame, assuming that the first given column has type String and all the others
    ...
    */
  def dfSchema(columnNames: List[String]): StructType = StructType(columnNames.zipWithIndex.map {
    case (name, index) if index == 0 => StructField(name, StringType, false)
    case (name, _) => StructField(name, DoubleType, false)
  })
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def matching(columnNames: List[String], matchers: List[String]): List[String] =
      columnNames.filter(name => matchers.exists(s => name.contains(s)))

    val primaryNeeds = matching(columnNames, List("t01", "t03", "t11", "t1801", "t1803"))
    val workingActivities = matching(columnNames.filter(elem => !primaryNeeds.contains(elem)), List("t05", "t1805"))
    val other = matching(
      columnNames.filter(elem => !primaryNeeds.contains(elem) && !workingActivities.contains(elem)),
      List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18"))
    (
      primaryNeeds.map(new Column(_)),
      workingActivities.map(new Column(_)),
      other.map(new Column(_))
    )
  }
  ...
  def timeUsageSummary(
    ...
    val sexProjection: Column = expr("IF(1 == tesex, 'male', 'female')").as("sex")
    val ageProjection: Column = expr("CASE WHEN teage > 55 THEN 'elder' WHEN teage > 22 THEN 'active' ELSE 'young' END").as("age")

    // Create columns that sum columns of the initial dataset
    // Hint: you want to create a complex column expression that sums other columns
    //       by using the `+` operator between them
    // Hint: don’t forget to convert the value to hours
    val primaryNeedsProjection: Column = expr(s"(${primaryNeedsColumns.mkString(" + ")}) / 60").as("primaryNeeds")
    val workProjection: Column = expr(s"(${workColumns.mkString(" + ")}) / 60").as("work")
    val otherProjection: Column = expr(s"(${otherColumns.mkString(" + ")}) / 60").as("other")
    df
  ...
  /** @return SQL query equivalent to the transformation implemented in `timeUsageGrouped`
    ...
    */
  def timeUsageGroupedSqlQuery(viewName: String): String = "SELECT working, sex, age, " +
  ...
  /**
    ...
    */
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] = timeUsageSummaryDf.map(row => {
    TimeUsageRow(
